...
- **Não confirmada:** implementação interna de `AdjustedAmount()` das classes concretas de bônus, `SpellBonus.MatchForType()`/`SpellPointBonus.MatchForType()`, critérios `Matches()`/`MatchesList()` e `SkillNamed()` usados pelos consumidores documentados em `GCS-Bonuses.md`.
