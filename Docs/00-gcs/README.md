...
- **Não confirmada:** mecanismos internos de matching usados por `Entity.BestSkillMatching()`, `SkillMatching()` e `SkillNamed()`. A seleção por `bestDefaultWithPoints()`/`bestDefault()`, `CalculateTechniqueLevel()` e a implementação de `SkillDefault.SkillLevel()`/`SkillLevelFast()` já estão confirmadas em `GCS-Skill-Level.md` e não integram mais esta questão aberta.
...
- **Não confirmada:** como cada classe concreta de bônus conecta ou especializa `LeveledAmount`; implementação de `SpellBonus.MatchForType()`/`SpellPointBonus.MatchForType()`, critérios `Matches()`/`MatchesList()` e `SkillNamed()` usados pelos consumidores documentados em `GCS-Bonuses.md`. O comportamento genérico de `LeveledAmount.AdjustedAmount()` já está confirmado e não integra mais esta questão aberta.
