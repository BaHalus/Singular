...
- **Confirmada:** a seleção de defaults de Skill e o cálculo de Technique estão rastreados em `GCS-Skill-Level.md`. `bestDefaultWithPoints()`/`bestDefault()` selecionam candidatos resolvíveis, evitam ciclos e preservam o default explicitamente escolhido nos casos documentados; `CalculateTechniqueLevel()` resolve o nível-base pelo caminho correspondente ao tipo de default, incorpora pontos e bônus observados e aplica o teto opcional implementado. Rastreabilidade: `model/gurps/skill.go` — `bestDefaultWithPoints`, `bestDefault`, `calcSkillDefaultLevel`, `inDefaultChain`, `resolveToSpecificDefaults`, `CalculateTechniqueLevel`.
- **Confirmada:** `SkillDefault.SkillLevel()` e `SkillLevelFast()` estão rastreados em `GCS-Skill-Level.md`: o despacho cobre defaults de Skill, Parry, Block, Dodge e atributos; `best()` recalcula candidatos por `Skill.CalculateLevel()`, enquanto `bestFast()` usa `Skill.LevelData.Level`; `isTLPermitted()` aplica a restrição de TL e `finalLevel()` aplica `Modifier` a níveis válidos. Rastreabilidade: `model/gurps/skill_default.go` — `SkillDefault`, `(*SkillDefault).SkillLevel`, `(*SkillDefault).SkillLevelFast`, `(*SkillDefault).best`, `(*SkillDefault).bestFast`, `(*SkillDefault).isTLPermitted`, `(*SkillDefault).finalLevel`.
- **Confirmada:** os mecanismos de matching de Skills usados por defaults e bônus estão rastreados em `GCS-Bonuses.md`: `SkillNamed()`/`SkillMatching()` aplicam filtros de exclusão e pontos, fazem matching por nome/especialização ou critérios, e `BestSkillNamed()`/`BestSkillMatching()` recalculam candidatos e conservam o maior nível. Rastreabilidade: `model/gurps/entity.go` — `(*Entity).SkillNamed`, `(*Entity).BestSkillNamed`, `(*Entity).SkillMatching`, `(*Entity).BestSkillMatching`, `skillSpecializationMatches`.
...
- **Confirmada:** `Weapon.SkillLevel()` avalia os `Defaults` por `SkillDefault.SkillLevelFast()`, ignora `fxp.Min`, aplica ajustes comuns e conserva o maior resultado com piso zero. Os ajustes observados incluem penalidade por ST mínima, bônus nomeados e `SkillBonus` locais de seleção `ThisWeapon`, além de penalidade de carga no caso melee com parry e fencing resolvidos como verdadeiros. `ResolveBoolFlag()` resolve `WeaponSwitch` por maioria, preservando o valor inicial em empate. Rastreabilidade: `model/gurps/weapon.go` — `(*Weapon).SkillLevel`, `(*Weapon).skillLevelBaseAdjustment`, `(*Weapon).skillLevelPostAdjustment`, `(*Weapon).EncumbrancePenalty`, `(*Weapon).extractSkillBonusForThisWeapon`, `(*Weapon).ResolveBoolFlag`.
...
- **Não confirmada:** como cada classe concreta de bônus conecta ou especializa `LeveledAmount`; implementação de `SpellBonus.MatchForType()`/`SpellPointBonus.MatchForType()` e critérios `Matches()`/`MatchesList()` usados pelos consumidores documentados em `GCS-Bonuses.md`. O comportamento genérico de `LeveledAmount.AdjustedAmount()` e o matching de Skills por `SkillNamed()`/`SkillMatching()` já estão confirmados e não integram mais esta questão aberta.
