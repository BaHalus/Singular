...
- **Não confirmada:** implementação interna de `SkillDefault.SkillLevel()`/`SkillLevelFast()` e mecanismos internos de matching usados por `Entity.BestSkillMatching()`, `SkillMatching()` e `SkillNamed()`. A seleção por `bestDefaultWithPoints()`/`bestDefault()` e `CalculateTechniqueLevel()` já está confirmada em `GCS-Skill-Level.md` e não integra mais esta questão aberta.
...
- **Não confirmada:** implementação interna de `SkillDefault.SkillLevelFast()`, `WeaponDamage.ResolvedDamage()`, métodos `Resolve()` das estruturas de combate, `Validate()` e pipeline integral de `collectWeaponBonuses()`. `Weapon.SkillLevel()`, seus ajustes diretamente observados e `ResolveBoolFlag()` já estão confirmados em `GCS-Weapons.md` e não integram mais esta questão aberta.
