...
- **Não confirmada:** resolução integral de defaults de Skill (`bestDefaultWithPoints()`/`bestDefault()`), algoritmo completo de `CalculateTechniqueLevel()` e implementação interna de `Entity.SkillPointBonusFor()`/`Entity.SkillBonusFor()`. `Skill.UpdateLevel()` e `CalculateSkillLevel()` já estão confirmados em `GCS-Skill-Level.md` e não integram mais esta questão aberta.
- **Não confirmada:** algoritmo interno de `Spell.UpdateLevel()` além de sua participação já confirmada no ciclo de convergência de `Entity.Recalculate()`.
