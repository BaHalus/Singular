...
Fonte principal: `model/gurps/entity.go`, tipos e métodos `Entity`, `(*Entity).Recalculate`, `(*Entity).ensureAttachments`, `(*Entity).DiscardCaches`, `(*Entity).processFeatures`, `(*Entity).processPrereqs`, `(*Entity).UpdateSkills` e `(*Entity).UpdateSpells`.
...
**Confirmada.** Cada execução de `processFeatures()` começa zerando `e.features`.
...
**Confirmada.** Após a coleta, `processFeatures()` atualiza `LiftingStrengthBonus`, `StrikingStrengthBonus`, `ThrowingStrengthBonus`, `Attribute.Bonus`, `Attribute.CostReduction`, `Profile`, `DodgeBonus`, `ParryBonus` e `BlockBonus` pelos métodos mostrados na implementação.

Rastreabilidade: `model/gurps/entity.go` — `(*Entity).processFeatures`, `(*Entity).processFeature`.
...
## Relação observada com desserialização

**Confirmada.** `(*Entity).UnmarshalJSONFrom()` chama `Recalculate()` ao final do fluxo observado de desserialização, depois das migrações/normalizações mostradas no método e da reconciliação de `PointsRecord` com `TotalPoints`.

Rastreabilidade: `model/gurps/entity.go` — `(*Entity).UnmarshalJSONFrom`, `(*Entity).Recalculate`.

## Limites desta passagem

- **Não confirmada:** implementação interna de `Skill.UpdateLevel()`; aqui está confirmado apenas seu lugar e seu sinal de mudança no ciclo.
- **Não confirmada:** implementação interna de `Spell.UpdateLevel()`; aqui está confirmado apenas seu lugar e seu sinal de mudança no ciclo.
- **Não confirmada:** semântica interna de `Equipment.ReallyEquipped()`; aqui está confirmado apenas que ela condiciona a coleta de Features de Equipment em `processFeatures()`.
- **Não confirmada:** efeito concreto de cada classe de Feature após o despacho além dos casos e atualizações diretamente observados em `entity.go`.
