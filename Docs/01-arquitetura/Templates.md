...
Templates representa pacotes de personagem preservados pela SINGULAR. O domínio não se limita a arquivos GCS: ele sustenta raças, modelos raciais, metacaracterísticas, profissões, arquétipos, corpos, formas e pacotes de campanha.
...
`id` é a identidade soberana da SINGULAR, única em `Character.templates`. `externalIds` preserva identidades externas sem substituir `id`. Nome e posição nunca definem vínculos.
...
Coleções anteriores, como `traits.advantages`, `skills`, `spells` e `equipment`, permanecem como projeções somente leitura reconstruídas a partir de `entries`.
...
- save/load preserva a forma canônica e o payload temporário de compatibilidade.
...
Templates e entradas são profundamente imutáveis. Dados recebidos são clonados, estruturas aninhadas não compartilham referências mutáveis e operações não modificam o pacote existente.
...
As operações atuais continuam usando projeções conhecidas:
...
O fluxo soberano de análise, plano, revalidação e aplicação pertence ao DOM-TEMPLATE-1.3.
...
preserva identidade, origem, entradas conhecidas e desconhecidas, pontos declarados, metadados, dados brutos e compatibilidade sem criar novos vínculos.
