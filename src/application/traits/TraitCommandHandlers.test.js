...
function runtime() {
  ...
  return {
    ...
    idGenerator: {
      next(prefix) {
        sequence += 1;
        return `${prefix}:trait-edit-${sequence}`;
      },
    },
  };
}
...
test("edits Traits immutably in the domain without recalculating points", () => {
...
test("rejects unsupported Trait patches and invalid reorder targets", () => {
  const traits = character().traits;

  assert.throws(
    () => updateTrait(traits, "trait-curious", { costFormula: "DX * 2" }),
    /unsupported fields/,
  );
  assert.throws(
    () => reorderTrait(traits, "trait-curious", 9),
    /target index is invalid/,
  );
  assert.throws(
    () => removeTrait(traits, "missing"),
    /Trait not found/,
  );
});
...
test("updates, removes and reorders Traits by id through handler entries", () => {
  const first = executeCommand(
    session(),
    command(TRAIT_COMMAND_TYPES.UPDATE, {
      traitId: "trait-combat-reflexes",
      patch: {
        name: "Reflexos em Combate revisado",
        source: { reference: "B43 revisado" },
        pointValue: { declaredPoints: 15 },
      },
    }),
    registry(),
    runtime(),
  ...
  const second = executeCommand(
    first.session,
    command(TRAIT_COMMAND_TYPES.REORDER, {
  ...
  const third = executeCommand(
    second.session,
    command(TRAIT_COMMAND_TYPES.REMOVE, {
      traitId: "trait-combat-reflexes",
    }, 2),
    registry(),
    runtime(),
  ...
  assert.equal(first.session.character.traits[0].name, "Reflexos em Combate revisado");
