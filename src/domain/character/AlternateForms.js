...
export function validateAlternateFormSets(sets) {
  ...
  for (const set of sets) {
    ...
    if (ids.has(set.id)) throw new Error("Alternate form set ids must be unique");
    ids.add(set.id);
  }
  return true;
}
...
export function validateAlternateFormSet(set) {
  ...
  for (const form of set.forms) {
    ...
    if (formIds.has(form.id)) throw new Error("Alternate form ids must be unique inside a set");
    formIds.add(form.id);
  }
  ...
  validateNullableObject(set.importMeta, "Alternate form set importMeta must be object or null");
  return true;
}
...
export function validateAlternateForm(form) {
  ...
  if (form.transitionRules !== null) validateFormTransitionRules(form.transitionRules);
  validateNullableObject(form.transitionRulesOverride, "Alternate form transitionRulesOverride must be object or null");
  validateNullableObject(form.transitionRulesResolution, "Alternate form transitionRulesResolution must be object or null");
  validateNullableObject(form.importMeta, "Alternate form importMeta must be object or null");
  return true;
}
...
function normalizeForms(value) {
  ...
  if (!Array.isArray(value)) throw new Error("Alternate form set forms must be array");
...
function validateNullableString(value, errorMessage) {
  if (value !== null && typeof value !== "string") throw new Error(errorMessage);
}
