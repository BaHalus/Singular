...
test("links the unique Morfose advantage and resolves active modifiers", () => {
  ...
  const resolution = resolveMorphProfile(
    character,
    "set-morph",
...
test("accepts the English technical trait name Morph", () => {
  const character = createMorphCharacter({
    advantages: [
      morphAdvantage({
        id: "adv-morph-english",
        name: "Morph",
        modifiers: [
          {
            id: "mod-unlimited",
  ...
  assert.equal(resolution.sourceTraitId, "adv-morph-english");
  assert.equal(resolution.morphProfile.pointLimitMode, "unlimited");
});
...
test("explicit morphProfile directives override builtin modifier mappings", () => {
  ...
  assert.equal(
    resolution.decisions.improvisation.mode.source,
    "explicit",
  );
});
...
test("manual override has final precedence and is persisted", () => {
  ...
  const applied = applyResolvedMorphProfile(
    character,
    "set-morph",
    {
  ...
  assert.equal(
    set.morphProfileResolution.decisions.pointLimit.overridden,
    true,
  );
});
...
test("same-priority explicit conflicts are diagnosed instead of silently merged", () => {
  const character = createMorphCharacter({
    advantages: [
      morphAdvantage({
        modifiers: [
...
test("campaign rule overrides builtin mappings but not manual override", () => {
  const character = createMorphCharacter({
    advantages: [
      morphAdvantage({
        modifiers: [
...
test("applies per-set overrides to all Morfose sets", () => {
  ...
  assert.equal(
    applied.character.alternateFormSets[0].morphProfile.pointLimit,
    40,
  );
  assert.equal(
