...
test("adds advantage without mutating original", () => {
  ...
  assert.equal(
    character.advantages.length,
    0
  );

  assert.equal(
    updated.advantages.length,
    1
  );
});
...
test("adds disadvantage without mutating original", () => {
  ...
  assert.equal(
    character.disadvantages.length,
    0
  );

  assert.equal(
    updated.disadvantages.length,
    1
  );
});
...
test("adds condition without mutating original", () => {
  ...
  assert.equal(
    character.state.conditions.length,
    0
  );

  assert.equal(
    updated.state.conditions.length,
    1
  );
});
