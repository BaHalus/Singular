...
export function findEquipmentItemIndex(equipment, itemId) {
  return findEquipmentItemIndexRecursive(equipment, itemId);
}

function findEquipmentItemIndexRecursive(equipment, itemId) {
  const index = equipment.findIndex(item => item.id === itemId);
  if (index >= 0) return index;
  for (const item of equipment) {
    const childIndex = findEquipmentItemIndexRecursive(item.children ?? [], itemId);
